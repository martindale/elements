...
bool CheckInputs(const CTransaction& tx, CValidationState &state, const CCoinsViewCache &view, bool fScriptChecks,
                 unsigned int flags, bool cacheStore, std::set<std::pair<uint256, COutPoint> >& setWithdrawsSpent,
...
public:
    CScriptCheck(const CCoins& txFromIn, const CTransaction& txToIn, unsigned int nInIn, const CTxOutValue& amountPreviousInputIn, CAmount nTxFeeIn, int nSpendHeightIn, unsigned int nFlagsIn, bool cacheIn) :
        scriptPubKey(txFromIn.vout[txToIn.vin[nInIn].prevout.n].scriptPubKey), amount(txFromIn.vout[txToIn.vin[nInIn].prevout.n].nValue),
        amountPreviousInput(amountPreviousInputIn),
        ptxTo(&txToIn), nIn(nInIn), nTxFee(nTxFeeIn), nSpendHeight(nSpendHeightIn), nFlags(nFlagsIn), cacheStore(cacheIn) { }
