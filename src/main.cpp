                              ...
                              std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        if (!CheckInputs(tx, state, view, true, scriptVerifyFlags, true, setWithdrawsSpent2)) {
            setWithdrawsSpent2.clear();
            if (CheckInputs(tx, state, view, true, scriptVerifyFlags & ~(SCRIPT_VERIFY_WITNESS | SCRIPT_VERIFY_CLEANSTACK), true, setWithdrawsSpent2)) {
                setWithdrawsSpent2.clear();
                if (!CheckInputs(tx, state, view, true, scriptVerifyFlags & ~SCRIPT_VERIFY_CLEANSTACK, true, setWithdrawsSpent2)) {
        ...
        if (!CheckInputs(tx, state, view, true, MANDATORY_SCRIPT_VERIFY_FLAGS, true, setWithdrawsSpent2))
        {
...
bool CScriptCheck::operator()() {
    ...
    if (!VerifyScript(scriptSig, scriptPubKey, witness, nFlags, CachingTransactionSignatureChecker(ptxTo, nIn, amount, amountPreviousInput, Params().GetConsensus().fedpegScript, cacheStore), &error)) {
        return false;
    }
    return true;
}
...
bool CheckInputs(const CTransaction& tx, CValidationState &state, const CCoinsViewCache &inputs, bool fScriptChecks, unsigned int flags, bool cacheStore, set<pair<uint256, COutPoint> >& setWithdrawsSpent, std::vector<CCheck*> *pvChecks)
{
    if (!tx.IsCoinBase())
    {
        ...
        if (fScriptChecks) {
            ...
            for (unsigned int i = 0; i < tx.vin.size(); i++) {
                ...
                // Verify signature
                CCheck* check = new CScriptCheck(*coins, tx, i, prevValueIn, tx.nTxFee, GetSpendHeight(inputs), flags, cacheStore);
                if (!QueueCheck(pvChecks, check)) {
                    if (flags & STANDARD_NOT_MANDATORY_VERIFY_FLAGS) {
                        ...
                        // non-upgraded nodes.
                        CScriptCheck check2(*coins, tx, i, prevValueIn, tx.nTxFee,
                                GetSpendHeight(inputs), flags & ~STANDARD_NOT_MANDATORY_VERIFY_FLAGS, cacheStore);
                        if (check2())
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, const CChainParams& chainparams, set<pair<uint256, COutPoint> >* setWithdrawsSpent, bool fJustCheck)
{
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        if (!tx.IsCoinBase())
        {
            ...
            if (!CheckInputs(tx, state, view, fScriptChecks, flags, fCacheResults, setWithdrawsSpent == NULL ? setWithdrawsSpentDummy : *setWithdrawsSpent, nScriptCheckThreads ? &vChecks : NULL))
    ...
    if (!VerifyAmounts(view, block.vtx[0], -blockReward), false)
