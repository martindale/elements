                                    ...
                                    unsigned int nIn, unsigned int flags, bitcoinconsensus_error* err)
{
    try {
        ...
        if (amountPreviousInput < -1 || (nIn != 0 && !MoneyRange(amountPreviousInput)))
            return VerifyScript(tx.vin[nIn].scriptSig, CScript(scriptPubKey, scriptPubKey + scriptPubKeyLen), nIn < tx.wit.vtxinwit.size() ? &tx.wit.vtxinwit[nIn].scriptWitness : NULL, flags, TransactionNoWithdrawsSignatureChecker(&tx, nIn, amount), NULL);
        else
